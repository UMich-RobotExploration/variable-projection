...
int main() {
  ...
  prob.setRank(5);
  prob.updateProblemData();  // ensure fresh
  ...
  std::cout << "init: " << init.rows() << "x" << init.cols() << " norm=" << init.norm() << "\n";
  ...
  std::cout << "CpuRTR: f=" << cpu_r.f << " iters=" << cpu_r.outer_iters << " stop=" << cpu_r.stop_reason << "\n\n";
  // GpuRTR
  ...
  std::cout << "GpuRTR: f=" << gpu_r.f << " iters=" << gpu_r.outer_iters << " stop=" << gpu_r.stop_reason << "\n";
}

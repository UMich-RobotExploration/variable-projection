                                      ...
                                      const VarPro::Problem &problem)
{
    ...
    while (std::getline(init_file, line))
    {
        ...
        if (tokens[0] == "VERTEX_POSE")
        {
            ...
            bool skip_translation =
                VarPro::isMarginalized(problem.getFormulation()) ||
    ...
    // The marginalized formulations (Implicit and Dense) optimize only the top
    // block; the translations are recovered analytically afterwards.
    if (VarPro::isMarginalized(problem.getFormulation()))
    {
